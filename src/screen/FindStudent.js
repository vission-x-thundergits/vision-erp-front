...
import StudentTable from "../components/reuse/StudentTable";
...
export default function FindStudent() {
  ...
  const findStudentWithClassAndSection = () => {
    fetch(`${BASE_URL}api/student/byClassOrSection/${studentClass}/${studentSection}`, {
      headers: {
          ...
          <div className="form-control">
            ...
            >
              ...
              {searchQueryOptions.map((queryType) => (
                <option key={queryType} value={queryType}>{queryType}</option>
              ))}
            </select>
          </div>
          <div className="form-control">
...
<br /><br />
      {/* Render the StudentTable component with the retrieved students */}
      {students.length > 0 && (
        <div>
